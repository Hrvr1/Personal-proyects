...
@section('title', 'Cartelera - ' . $cine->nombre)

@section('content')

<link rel="stylesheet" href="{{ asset('css/peliculas.css') }}">
    <div class="cine-container">
        <h1 class="titulo">Cartelera de {{ $cine->nombre }} - {{ $cine->localidad }}</h1>
        {{-- Barra de búsqueda y filtros --}}
        <form method="GET" action="{{ route('cartelera.show', $cine->id) }}" class="search-form container">
            <div class="row g-3">
                {{-- Buscador por nombre --}}
                ...
                <div class="col-12 col-md-6">
                    <input type="date" id="dia_sesion" name="dia_sesion" value="{{ request('dia_sesion') }}" class="form-control" />
                </div>
                <div class="col-12 col-md-6">
                    <input type="time" id="hora_sesion" name="hora_sesion" value="{{ request('hora_sesion') }}" class="form-control" />
                </div>
                ...
                <div class="col-12 col-md-6 d-flex gap-2">
                    ...
                    <a href="{{ route('cartelera.show', $cine->id) }}" class="btn btn-danger w-100">Quitar Filtros</a>
                </div>
            </div>
        </form>
                ...
                @foreach ($peliculas as $pelicula)
                    <div class="pelicula-card">
                        ...
                        <div class="pelicula-info">
                            <h2>
                                {{ $pelicula->nombre }}
                                @if($pelicula->reviews_avg_puntuacion)
                                    <span style="font-size: 1rem; color: #111; background: #ffe066; border-radius: 8px; padding: 2px 8px; margin-left: 8px; display: inline-block;">
                                        {{ number_format($pelicula->reviews_avg_puntuacion, 1) }} ★
                                    </span>
                                @endif
                            </h2>
                            <p><strong>Género:</strong> {{ $pelicula->genero }}</p>
                            <p><strong>Duración:</strong> {{ $pelicula->duracion }} minutos</p>
                            <p class="descripcion-corta"><strong>Descripción:</strong> {{ Str::limit($pelicula->descripcion, 200) }}</p>
...
<script>
    document.getElementById('toggle-genero-btn').addEventListener('mousedown', function (e) {
